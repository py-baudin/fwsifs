...
def solve_npt_pdff(times):
    ...
    def jac(args):
        ...
        class Jac(LinearOperator):
            def __init__(self, grad):
                self.grad = grad
                self.shape = (2 * self.grad.size, r2.size)
                self.dtype = self.grad.real.dtype
            def _matvec(self, vec):
                vec = np.reshape(vec, (1,) + self.grad.shape[1:])
                prod = (self.grad * vec).ravel()
                return np.r_[prod.real, prod.imag]
            def _rmatvec(self, vec):
                vec = vec[:len(vec)//2] + 1j * vec[len(vec)//2:]
                vec = np.reshape(vec, grad.shape)
                prod = np.sum(grad.conj() * vec, axis=0)
                return prod.ravel().real
...
sns.move_legend(ax2, 'upper left', bbox_to_anchor=(1, 1), fontsize='small', title=r'${\Delta\omega}_\chi$ [ppm]')
plt.xlabel('$PDFF$ [%]')
plt.ylabel('')
